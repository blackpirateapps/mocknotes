...
import { processImagesInBackground } from '../lib/queue'; // Import the new queue
...
export default function Importer() {
  ...
  const startProcessing = async () => {
    ...
    try {
      // This now creates a DB entry and returns immediately
      // The actual Gemini call happens in the background
      await processImagesInBackground(previews);
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <div className="bg-white rounded-xl shadow-things border border-things-border p-8">
          ...
          <button
            ...
            className="w-full bg-things-blue text-white font-medium py-3 rounded-lg shadow-md hover:bg-blue-600 transition flex items-center justify-center gap-2 disabled:opacity-50"
          >
            {loading ? (
              <>
              ...
              <>
                ...
                <span>Analyze {previews.length} Image{previews.length !== 1 ? 's' : ''}</span>
              </>
            )}
          </button>
        </div>
